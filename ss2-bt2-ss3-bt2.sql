...
on p.pid = od.p_id
where od.p_id is null;
-- Hiển thị tất cả đơn hàng mua trên 2 sản phẩm
select o.oid, o.c_id, o.oDate, o.oTotalprice,COUNT(od.p_id) AS ProductCount
from orderPro o
join orderdetail od ON o.oid = od.o_id
group by o.oid, o.c_id, o.oDate, o.oTotalprice
having count(od.p_id) > 2;
-- Hiển thị đơn hàng có tổng giá tiền lớn nhất
select o.oid,o.c_id,p.pName, sum(od.odQTY*p.pprice) 'tong tien hoa don' from orderpro o join orderdetail od
on o.oid = od.o_id
join product p 
on p.pid = od.o_id 
group by od.o_id
order by 'tong tien hoa don' desc limit 1; 
